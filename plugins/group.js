module.exports = {
    ...
    async run(LilyBot, m, { command, text, q, isRegistered, isBotAdmins, DinzTheCreator, groupMetadata, participants, mess, reply, replyviex, prefix }) {
        ...
        switch (command) {
            ...
            case 'tagall': {
                ...
                LilyBot.sendMessage(m.chat, { text: teks, mentions: participants.map(a => a.id) }, { quoted: m });
                break;
            }
            case 'promote': {
                if (!isBotAdmins) return reply(mess.only.Badmin);
                let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
                await LilyBot.groupParticipantsUpdate(m.chat, [users], 'promote');
            ...
            case 'demote': {
                if (!isBotAdmins) return reply(mess.only.Badmin);
                let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
                await LilyBot.groupParticipantsUpdate(m.chat, [users], 'demote');
            ...
            case 'group': {
                if (!isBotAdmins) return reply(mess.only.Badmin);
                if (text === 'open') {
                    await LilyBot.groupSettingUpdate(m.chat, 'not_announcement');
                    reply('Grup Dibuka');
                } else if (text === 'close') {
                    await LilyBot.groupSettingUpdate(m.chat, 'announcement');
                    reply('Grup Ditutup');
                } else {
