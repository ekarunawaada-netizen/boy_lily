module.exports = {
    ...
    async run(LilyBot, m, { command, text, prefix, pushname, isRegistered, replydaftar, replyviex, getBuffer }) {
        if (!isRegistered) {
        ...
        switch (command) {
            ...
            case 'menfes':
            case 'menfess': {
                const session = Object.values(LilyBot.menfes).find(v => v.state === 'CHATTING' && [v.a, v.b].includes(m.sender));
                if (session) {
                    const target = session.a === m.sender ? session.b : session.a;
                    await LilyBot.sendMessage(target, {
                ...
                const roof = Object.values(LilyBot.menfes).find(menpes => [menpes.a, menpes.b].includes(m.sender));
                if (roof) {
                ...
                await LilyBot.sendMessage(`${nomorNya}@s.whatsapp.net`, {
                    image: tod,
                    caption: yoi
                });
            ...
            case 'balasmenfess': {
                const room = Object.values(LilyBot.menfes).find(room => [room.a, room.b].includes(m.sender) && room.state === 'WAITING');
                if (!room) {
                ...
                room.b = m.sender;
                room.state = 'CHATTING';
                await LilyBot.sendMessage(other, {
            ...
            case 'tolakmenfess': {
                ...
                delete LilyBot.menfes[roof.id];
                break;
            }

            case 'stopmenfess': {
                const find = Object.values(LilyBot.menfes).find(menpes => [menpes.a, menpes.b].includes(m.sender));
                if (!find) return replyviex("Belum ada sesi menfess");
                const to = find.a === m.sender ? find.b : find.a;
                await LilyBot.sendMessage(to, {
                    text: "_Sesi menfess ini telah dihentikan._",
                    mentions: [m.sender]
                });
                replyviex("Sesi menfess dihentikan.");
                delete LilyBot.menfes[find.id];
                break;
            }
        }
    }
