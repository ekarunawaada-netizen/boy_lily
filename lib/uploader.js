...
async function SupabaseUpload(file, bucket = 'Bot_asset') {
	try {
		...
		const { data, error } = await supabase.storage
			...
			.upload(fileName, buffer, {
				contentType: (await fromBuffer(buffer))?.mime || 'application/octet-stream',
				upsert: true
			});
...
module.exports = { CatBox, UploadFileUgu, webp2mp4File, floNime, SupabaseUpload }
